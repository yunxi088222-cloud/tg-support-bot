...
TOKEN = "8148731949:AAHBcTPPeJ89kmUbn_PJqGIgs96XyOulzS0"   # ← 改成你测通的那个
GROUP_ID = -1002971903995   # ← 改成你真实群 ID
...
async def user_to_group(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await m.copy(chat_id=GROUP_ID, caption=caption, message_thread_id=topic_id, parse_mode="HTML")
    try:
...
async def group_to_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    m = update.message
    if not m.is_topic_message or m.chat.id != GROUP_ID: return
    if m.from_user.id == (await context.bot.get_me()).id: return
    user_id = m.message_thread_id
    try: await m.copy(chat_id=user_id)
    except: pass
